...
4. **Cmdline model flags.** The cmdline is scanned for known model flags. Recognized flags: `--model`, `--model-name`, `--model-id`, `--model-path`, `--repo-id`, `--hf-model`, `--hf-repo`, and the short form `-m`. Both `--flag value` and `--flag=value` are accepted. Two value shapes are matched:
   - **HuggingFace repo id** (`org/repo`, e.g. `mlx-community/Qwen2.5-VL-7B-Instruct-4bit`). The value must contain exactly one `/`, with both halves non-empty and using only `[A-Za-z0-9._-]`. This catches MLX/`mlx-vlm`/`transformers` workloads which read safetensors into the Metal heap and then close the file descriptor — i.e. the cases where layer 2 has nothing to attribute.
   - **Bare model label** (no slash, e.g. `Qwen3.6-35B-A3B-6bit`, `llama-3.2-3b-instruct`). Used by benchmark clients (BFCL, lm-eval, …) that pass an opaque label which the server resolves. Must be ≥ 3 chars, contain only `[A-Za-z0-9._-:]`, and include at least one of `[0-9._-:]` so generic English words (`auto`, `default`, `latest`) don't get promoted. The short `-m` flag is excluded here because python overloads it for module names (`python -m pkg.subpkg`).

For sources 3 and 4 the on-disk size is unknown, so `size_bytes` is left empty. For HF repo ids and file paths, `resident_bytes` is filled with the matched process's RSS — unless source 2 already produced a per-file entry for the same pid, in which case those per-file numbers win and the cmdline entry stays empty to avoid double-counting in the TOTAL row. For bare labels `resident_bytes` is left empty, because the caller is just as likely a client process whose RSS doesn't represent loaded weights.
